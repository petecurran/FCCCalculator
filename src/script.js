function App(){
  ...
  const buttonDisplay = buttons.map((item) =>
    <CalcButton key={item.id} id={item.id} label={item.label} handleClick={handleClick}/>
  );
  ...
  return(
    <div id="calculator">
      <h4 id="brand">SACIO</h4>
      <div id="combinedDisplay">
        <h4 id="prevDisplay">{prevDisplay}</h4>
...
function CalcButton(props){
  return(
    <button id={props.id} className="grid-item calcButton" onClick={() => props.handleClick(props.label)}>
      {props.label}
    </button>
  )
